...
var quanxian_id_his = sessionStorage.getItem('quanxian_id_his');
var quanxian_name_his = sessionStorage.getItem('quanxian_name_his');
var shenqingjilu = sessionStorage.getItem('quanxian_jilu_his');
	...
	if(userId == "" || userId == undefined || userId == null) {
		...
		var reque = function() {
			return {
				...
				userid: quanxian_id_his,
				recordid: shenqingjilu
			}
		}
		rq1 = reque();
		var req = function() {
		...
		$.ajax({
			...
			type: "POST",
			url: urltest + 'verify/merchant/selectMerchantPrivilegeRecordList',
			data: req3,
			async: false,
			dataType: "json",
...
function createShowingTable(bb) {
	...
	if(bb.total == 0) {
		...
		if(len == 0) {
			...
			for(var i = 0; i < len; i++) {
				...
				tableStr = tableStr + '<tr>' +
					'<th>' + num + '</th>' +
					'<th>' + quanxian_name_his + '</th>' +
					'<th>' + bb[i].areaname + '</th>' +
					'<th>' + bb[i].createdate + '</th>' +
					'<th>' + bb[i].auditresult + '</th>' +
					'<th>' + bb[i].auditusername + '</th>' +
					'<th>' + bb[i].auditdate + '</th>' +
		...
		$("#tbbb tr th:nth-child(8)").click(function() {
			...
			var beizhu = $(this).siblings().eq(7).html();
			$("#myModalLabel").html(mname + "，小区权限审核历史记录");
			$("#s_beizhu").val(beizhu);
		});
	}
}

...
function table_jia() {
	...
	$.ajax({
		...
		type: "POST",
		url: urltest + 'verify/function/selectPushList',
		data: req3,
		async: false,
		dataType: "json",
...
$(".message_add").click(function() {
	//	debugger
	var pushid = $('#x_tuisong').val();
	var type = $('#x_leixing').val();
	var pushtarget = $('#x_tuisongrenqun').val();
	if(type == "3") {
		var title = $('#x_message_title').val();
		var info = $('#x_message_info').val();
		if(title == "") {
			alert("请输入标题");
		} else if(info == "") {
			alert("请输入内容");
			...
			$.ajax({
				contentType: "application/json; charset=utf-8",
				type: "POST",
				url: urltest + 'verify/function/addSysPush',
				data: req3,
				async: true,
				dataType: "json",
				success: function(msg) {
					...
					if(msg.responseCode == 200) {
						console.log(msg);
						alert("增加推送成功");
						location.href = "xiaoxituisong.html"
					} else {
						alert(msg.resoponseDisp)
					}
				},

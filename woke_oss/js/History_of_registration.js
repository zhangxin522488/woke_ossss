...
var zhuce_id_his = sessionStorage.getItem('zhuce_id_his');
var zhuce_name_his = sessionStorage.getItem('zhuce_name_his');
...
//		alert("请登陆之后再操作");
		...
		$.ajax({
			...
			type: "POST",
			url: urltest + 'verify/merchant/selectMerchantAuditList',
			data: req3,
			async: false,
			dataType: "json",
	...
	if(bb.total == 0) {
		...
		if(len == 0) {
			...
			for(var i = 0; i < len; i++) {
				...
				if(bb[i].auditresult==undefined){
					bb[i].auditresult="";
				}else if(bb[i].auditresult=="-1"){
					bb[i].auditresult="审核不通过";
				}else if(bb[i].auditresult=="1"){
					bb[i].auditresult="审核通过";
				}else if(bb[i].auditresult=="0"){
					bb[i].auditresult="待审核";
				}
		...
		$("#tbbb tr th:nth-child(6)").click(function() {
			...
			var beizhu = $(this).siblings().eq(5).html();
			$("#myModalLabel").html(mname + "，历史记录");
			$("#s_beizhu").val(beizhu);
		});
	}
}

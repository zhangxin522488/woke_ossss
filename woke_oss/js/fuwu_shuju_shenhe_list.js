...
$(function() {
	if(userId == "" || userId == undefined || userId == null) {
		...
		$.ajax({
			...
			type: "POST",
			url: urltest + 'verify/merchant/selectDataPerAuditList',
			data: req3,
			async: false,
			dataType: "json",
...
function createShowingTable(bb) {
	...
	if(bb.total == 0) {
		...
		$("#tbbb tr th:nth-child(7)").click(function() {
			...
			var s_xiaoqun = $(this).siblings().eq(3).html();

			var s_quanxian = $(this).siblings().eq(4).html();
			var s_shenher = $(this).siblings().eq(5).children("button").html();
			//小区ID
			var s_jiluID = $(this).siblings().eq(7).html();
			var s_shenqingjiluid = $(this).siblings().eq(8).html();
			var s_nameshenheID = $(this).siblings().eq(9).html();
			var s_shhename = $(this).siblings().eq(10).html();
			var s_shenheshijian = $(this).siblings().eq(11).html();
			var s_pizhu = $(this).siblings().eq(12).html();
			if(s_pizhu == 'undefined') {
				s_pizhu = ""
			}
			$("#myModalLabel").html(s_name + "权限审核详情");
			$('#s_name').html(s_name);
			$('#s_id').html(s_id);
			$('#s_xiaoqun').html(s_xiaoqun);
			$('#s_quanxian').html(s_quanxian);

			$('#s_shenher').html(s_shenher);
			$('#s_jiluID').html(s_jiluID);
			$('#s_nameshenheID').html(s_nameshenheID);
			$('#s_shhename').html(s_shhename);
			$('#s_shenheshijian').html(s_shenheshijian);
		...
		$("#tbbb tr th:nth-child(8)").click(function() {
			...
			var shenqingjilu = $(this).siblings().eq(8).html();
			sessionStorage.setItem("shuju_id_his", id);
			sessionStorage.setItem("shuju_name_his", name);
			sessionStorage.setItem("shuju_jilu_his", shenqingjilu);

			location.href = "History_shuju_quanxian_list.html";
		})
...
$(".shnehe_pass").click(function() {
	...
	if(auditSummary == "" || auditSummary == undefined || auditSummary == null) {
		...
		$.ajax({
			contentType: "application/json; charset=utf-8",
			type: "POST",
			url: urltest + 'verify/merchant/updateDataPerAuditPass',
			//			merchant/updateDataPerAuditPass
			...
			success: function(msg) {
				...
				if(msg.responseCode == 200) {
					console.log(msg);
					alert("审核成功！");
					location.href = "fuwu_shuju_shenhe_list.html";
				} else {
					alert(msg.resoponseDisp);
...
$(".shnehe_nopass").click(function() {
	...
	if(auditSummary == "" || auditSummary == undefined || auditSummary == null) {
		...
		$.ajax({
			contentType: "application/json; charset=utf-8",
			type: "POST",
			url: urltest + 'verify/merchant/updateDataPerAuditNotPass',
			data: req3,
			async: true,
			dataType: "json",
			success: function(msg) {
